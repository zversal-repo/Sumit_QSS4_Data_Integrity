...
public class AgreementService {
	...
	public static HashMap<Long, HashSet<Long>> getAgreements() throws IOException, SQLException {
		String query = "SELECT DISTINCT service_id,agreement_id FROM agreement_service ORDER BY service_id,agreement_id";
		return QueryProcessor.getMap(query, "service_id", "agreement_id", 1L, 1L);

	}

	// Function for getting all available agreements for a service_id in the
	// database
	public static HashSet<Long> getAgreements(String service_id) throws IOException, SQLException {
		String query = "SELECT DISTINCT agreement_id FROM agreement_service WHERE service_id=" + service_id
				+ " ORDER BY agreement_id";
		return QueryProcessor.getList(query, "agreement_id", 1L);

	}

	public static HashMap<Long, HashSet<Long>> getProductAgreements() throws IOException, SQLException {
		...
		return QueryProcessor.getMap(query, "product_id", "agreement_id", 1L, 1L);
	}
}
